with recursive
    step_cte  as (
	select 0 step, 0 it, x, y, h,
           (select sum(h) from input11) sm,
	   '.' flashes from input11
	union all	
	(
	  with recursive  
		  inp (step, x, y, h) as (
		  ...
		  select step, it, x, y, h, sm, flashes
		  from (
			select step, it, x, y, flashes,
			       case when h > 9 then 0 else h end h, 
			       sum(case when h > 9 then 0 else h end) over (partition by it) sm,
			       max(it) over () last_it
			from (select * from iter) iter_cte
		  ) tmp
		  where last_it = it and sm <> 0 
	  )
    )    
select max(step) + 1 as answer
from step_cte
